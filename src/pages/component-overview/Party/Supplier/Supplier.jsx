...
import {
  ...
  Update_Supplier,
  Delete_Supplier
} from '../../../../Redux/Action/Party/SupplierActions/SupplierActions';

const Supplier = () => {
  const dispatch = useDispatch();
  const { suppliers, loading, actionLoading } = useSelector((state) => state.PRT.supplier);
  ...
  useEffect(() => {
    dispatch(Fetch_Suppliers(2)); // 2 = Supplier ( Party Type ID )
  }, [dispatch]);
  ...
  const handleSubmit = async (values) => {
    let success = false;
    if (editingSupplier) {
      success = await dispatch(Update_Supplier(editingSupplier.id, values));
    } else {
      success = await dispatch(Add_Supplier(values));
    }
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          />
          <Popconfirm
            title={<span style={{ fontWeight: 600 }}>Delete Supplier</span>}
    ...
    return (
      <div style={{ background: 'white', borderRadius: 8, overflow: 'hidden' }}>
        ...
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: 'minmax(220px, 1fr) minmax(180px, 1fr) minmax(250px, 1.5fr) minmax(150px, 1fr) minmax(100px, 0.7fr)',
            background: '#f9fafb',
            padding: '16px',
        ...
        >
          {['NAME', 'CONTACT', 'ADDRESS', 'BALANCE', 'ACTIONS'].map((title, index) => (
      ...
      <MainCard
        ...
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <div style={{ fontSize: '20px', fontWeight: 600, color: '#111827' }}>Suppliers</div>
              <div style={{ fontSize: '13px', color: '#6b7280', fontWeight: 400 }}>Manage your suppliers</div>
            </div>
          </div>
        }
        secondary={
      ...
      >
        {loading ? (
          ...
          <Table
            ...
            locale={{
              emptyText: (
                <Empty description={<span style={{ color: '#9ca3af' }}>No suppliers found</span>} image={Empty.PRESENTED_IMAGE_SIMPLE} />
              )
            }}
            size="middle"
            style={{
